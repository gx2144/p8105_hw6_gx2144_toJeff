...
* Description: The distribution of Adjusted R square shows that this distribution has a heavy tail extending to low values and a bit of a “shoulder”, features that may be related to the frequency with which large outliers are included in the bootstrap sample.From the table above, we can see that the 95% confidence interval for adjusted R squared is [`r CI_adjr$conf.int`].
...
* Description: The distribution of log(β0 ∗ β1) shows that this distribution seems normal, features that may be related to the few outliers are included in the bootstrap sample. This indicates and transformation of certain estimates could solve the non-normality problem caused by outliers. From the table above, we can see that the 95% confidence interval for log(β0 ∗ β1) is [`r CI_log$conf.int`].
